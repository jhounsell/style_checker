function checkTextRules(userText) {
  ...
  const regexPatterns = [
    ...
    { regex: /\bdata\s((?!\w+(s|ing|lly)\b|collection|that|the|from|which)|do\b|are|have|go\b|arise\b|become\b|begin\b|break\b|come\b|fall\b|fly|grow\b|run\b|rise\b|sink\b|stand\b|strike\b|take\b|undergo\b|withdraw)/gi, message: "Treat as a singular noun: The data is stored on a secure server.", title: "data" },
    ...
    { regex: /\b(?:\d{1,2}\s)?may(?=\s\d{4}|\b)(?:\s\d{4})?\b/g, message: "Upper-case the first letter on months.", title: "months (case)" },
